...
async function startEventListener() {
  try {
    ...
    console.log(`✅ Đã kết nối với contract: ${process.env.NFT_CONTRACT_ADDRESS}`);
    ...
    console.log("🔄 Sử dụng polling để theo dõi Transfer events (mỗi 3 giây)...\n");
    startPolling();
      ...
      if (currentBlock > lastCheckedBlock) {
        ...
        for (const event of events) {
          ...
          if (from === ethers.ZeroAddress) {
            console.log("ℹ️  Đây là sự kiện MINT, bỏ qua (đã xử lý trong API /mint)");
            continue;
          }
