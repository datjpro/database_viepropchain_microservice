...
app.get("/health", async (req, res) => {
  try {
    ...
    try {
      const ipfsRes = await axios.get(`${IPFS_SERVICE_URL}/health`, {
        timeout: 2000,
      });
...
app.post("/properties/:id/mint", async (req, res) => {
  try {
    ...
    const mintResponse = await axios.post(`${BLOCKCHAIN_SERVICE_URL}/mint`, {
      recipient,
      tokenURI,
    });
    ...
    const { tokenId, transactionHash, blockNumber, mintedBy } =
      mintResponse.data.data;
...
app.get("/properties/stats/overview", async (req, res) => {
  try {
    const [totalProperties, totalMinted, byType, byStatus] = await Promise.all([
