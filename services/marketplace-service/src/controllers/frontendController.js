  ...
  try {
    ...
    const formattedListings = listings.map((listing) => ({
      ...
      address: {
        ...
        full: `${listing.propertyAddress?.ward || ""}, ${
          listing.propertyAddress?.district || ""
        }, ${listing.propertyAddress?.city || ""}`.replace(/^,\s*|,\s*$/g, ""),
      },
      ...
      price: {
        ...
        formatted: `${(parseFloat(listing.price.amount) / 1e18).toFixed(
          2
        )} ETH`,
      },
      seller: {
        address: listing.seller.walletAddress,
        shortAddress: `${listing.seller.walletAddress.slice(
          0,
          6
...
exports.getFeaturedListings = async (req, res) => {
  try {
    ...
    const formatted = featuredListings.map((listing) => ({
      ...
      propertyName: listing.propertyName,
      propertyType: listing.propertyType,
      address: `${listing.propertyAddress?.ward || ""}, ${
        listing.propertyAddress?.district || ""
      }, ${listing.propertyAddress?.city || ""}`.replace(/^,\s*|,\s*$/g, ""),
      image: listing.propertyImages?.[0] || "/placeholder-property.jpg",
      price: {
...
exports.getSearchSuggestions = async (req, res) => {
  try {
    ...
    const allSuggestions = [
      ...result.propertyNames.filter(
        (name) => name && name.toLowerCase().includes(q.toLowerCase())
      ),
      ...result.cities.filter(
        (city) => city && city.toLowerCase().includes(q.toLowerCase())
      ),
      ...result.districts.filter(
        (district) =>
          district && district.toLowerCase().includes(q.toLowerCase())
      ),
    ].slice(0, 10);
