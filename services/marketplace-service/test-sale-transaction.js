...
async function testSaleTransaction() {
  try {
    ...
    console.log(`   transferHistory length: ${nftBefore.transferHistory?.length || 0}`);
    ...
    if (updateResult.modifiedCount > 0) {
      ...
      console.log(`   saleHistory length: ${nftAfter.saleHistory?.length || 0}`);
      console.log(`   transferHistory length: ${nftAfter.transferHistory?.length || 0}`);

      // Check latest sale
      const latestSale = nftAfter.saleHistory[nftAfter.saleHistory.length - 1];
      const latestTransfer = nftAfter.transferHistory[nftAfter.transferHistory.length - 1];
      
      console.log(`\n📈 Latest sale:`);
      console.log(`   Price: ${latestSale?.price || 'N/A'} ${latestSale?.currency || ''}`);
      console.log(`   Date: ${latestSale?.date || 'N/A'}`);
      
      console.log(`\n📋 Latest transfer:`);
      console.log(`   Type: ${latestTransfer?.transferType || 'N/A'}`);
      console.log(`   Price: ${latestTransfer?.price || 'N/A'} ${latestTransfer?.currency || ''}`);
      
      // Verify data integrity
      const hasProperSale = nftAfter.totalSales > 0 && 
